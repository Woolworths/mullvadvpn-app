...
impl SystemdResolved {
    ...
    pub fn set_dns(&mut self, interface_name: &str, servers: Vec<IpAddr>) -> Result<()> {
        if let Some(link_object_path) = self.interface_links.get(interface_name).cloned() {
            self.set_link_dns(link_object_path, servers)
        } else {
            let link_object_path = self.fetch_link(interface_name)?;

            self.set_link_dns(link_object_path.clone(), servers)?;
            self.interface_links
                .insert(interface_name.to_owned(), link_object_path);
